...
def fetch_data(frame_idx):
    cam_rgb_points = dataset.get_cam_points_in_image_with_rgb(frame_idx, downsample_voxel_size= 'void_and_cluster')
    ...
    train_config['max_epoch']):
        ...
        for batch_idx in range(0, NUM_TEST_SAMPLE-batch_size+1, batch_size):
            ...
            for gi in range(COPY_PER_GPU*NUM_GPU):
                ...
                print('Initial_vertex_features:')
                print(t_initial_vertex_features)
                print(type(t_initial_vertex_features))
                input('Hold on...')
